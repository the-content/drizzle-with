...
export const withAutoFilters = (qb: Qb, values: Record<string, any>, overrides?: Record<string, FilterOps>) => {
    ...
    // 将values的键也转换为snake_case
    const snakeValues: Record<string, any> = {};
    for (const key in values) {
        snakeValues[camelToSnake(key)] = values[key];
    }
    ...
    for (const column in columns) {
        ...
        const value = snakeValues[snakeColumn];
        if (typeof value === 'undefined' || value === null) {
            continue;
        }
        if (snakeOverrides[snakeColumn]) {
            filters.push({ id: snakeColumn, op: snakeOverrides[snakeColumn], value: value });
            continue;
        }
        filters.push({ id: snakeColumn, op: 'eq', value: value });
    }
